...
def calculate_k_resolved_dos(k_points, bands, energy_range, delta_e, sigma):
    ...
    return (energies, dos_map_spin_up, dos_map_spin_down)
...
def weight_kpoint(k_point, U, Ef, E=0, a=3.98E-10, d=5E-9):  # 给归一化的相对坐标
    ...
    kappa_00 = (np.sqrt(2 * m * m_eff * eV * (U - Ef + E))) / h_bar
...
def draw_weights(k_points):
    ...
    for k_point in k_points:
        weight.append(weight_kpoint(k_point, 1.0, 0, 0, a=3.98E-10, d=6E-9))
        kx.append(k_point[0])
        ky.append(k_point[1])
...
kx, ky, weight = draw_weights(k_points)
