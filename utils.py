...
def fit_and_plot_1(file_name, data, B_max, fit_result, out_path, test_condition):
    ...
    buff = 0.001*f2[0] / (f1[0] *2)
    eff_field = -2 * buff  # v_2w的单位是uV，1w的单位是mV，二次的相当于数值增大了1000倍因此除以1000
    ...
    for i in range(len(fit_result) - 1):
        if fit_result[i]['current'] == fit_result[i + 1]['current'] and fit_result[i]['Mz'] == fit_result[i + 1]['Mz']:
            if fit_result[i]['harm_order'] == '1' and fit_result[i + 1]['harm_order'] == '2':
                delta_Bx_1 = 0 - 2 * fit_result[i + 1]['fit_value'] / (2*fit_result[i]['fit_value'])
...
# 保存分别测量的数据
def B_DL_result_save(delta_Bx, path):
    full_path = path + '\\result' + '.txt'
    file = open(full_path, 'w')
    for i in range(len(delta_Bx) - 1):
        if delta_Bx[i]['Mz'] == 'up':
            current, B_DL = delta_Bx[i]['current'], delta_Bx[i]['deltaB_x']
            file.write(str(current) + '    ' + str(B_DL))
            file.write('\n')
    file.write('\n' + '\n')
    for i in range(len(delta_Bx) - 1):
        if delta_Bx[i]['Mz'] == 'down':
            current, B_DL = delta_Bx[i]['current'], delta_Bx[i]['deltaB_x']
            file.write(str(current) + '    ' + str(B_DL))
            file.write('\n')
    file.close()
