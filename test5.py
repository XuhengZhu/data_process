...
# 创建更平滑的数据插值函数
def smooth_data(kx_sparse, ky_sparse, E_sparse, k_range, method='cubic', grid_num=200):
...
def draw(kx,ky,weight,k_range):
    fig, ax = plt.subplots(figsize =(7,6))
    im = ax.imshow(weight, extent=(-1*k_range,k_range,-1*k_range,k_range), cmap = 'viridis', interpolation='bilinear', origin='lower')
    cbar = fig.colorbar(im, ax=ax, orientation = 'vertical')
    cbar.set_label('Weight')
    ax.set_title('weight for k //')
    ax.set_xlabel('kx')
    ax.set_ylabel('ky')
    # 保存图片
